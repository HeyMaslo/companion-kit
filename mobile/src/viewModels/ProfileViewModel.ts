...
export class ProfileViewModel {
    ...
    readonly rewardInfoBlockVM = process.appFeatures.CLIENT_REWARDS_ENABLED ? new RewardsViewModel() : null;
    ...
    private get account() { return AppController.Instance.User.activeAccount; }

    public get name() { return AppController.Instance.User.displayName; }

    private get records() { return AppController.Instance.User?.records; }
    private get recordsLastWeek() { return AppController.Instance.User?.recordsLastWeek; }

    get wordsWeekly() { return this.recordsLastWeek.words; }

    get wordsTotal() { return this.records.words; }

    get pictureViewVM() { return this._pictureViewVM; }

    @computed
    get totalJournals() {
        const journalsCount = arrayCount(this.records.items, r => r.type === 'journal');
        return journalsCount;
    }

    @computed
    get totalSessions() {
        const sessionsCount = arrayCount(this.records.items, r => r.type === 'session');
        return sessionsCount;
    }

    @computed
    get totalWordsLength() {
        return this.records.items.reduce((acc, curr) => acc + (curr.entities.length || 0), 0);
    }

    @computed
    get avatarURL() { return (this.user && this.user.photoURL) || (this.authUser && this.authUser.photoURL); }
    ...
    public get coachName() {
        ...
        return this.coachProfile?.value.displayName || this.account?.coachName || '';
    }
    public get coachAvatarUrl() { return this.coachProfile?.value?.photoURL; }
