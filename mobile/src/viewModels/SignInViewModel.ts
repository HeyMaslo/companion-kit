...
export class SignInViewModel extends SignInViewModelBase {
    ...
    protected get Auth() {
        return AppController.Instance.Auth;
    }
    ...
    get enableAppleButton() {
        return AppController.Instance.Auth.appleSignInSupported;
    }

    get verificationCodeValue() {
        return this._verificationCodeValue;
    }
    set verificationCodeValue(verificationCodeValue: string[]) {
        this._verificationCodeValue = verificationCodeValue;
    }
    get errorMsg() {
    ...
    get inputRefArray() {
        return this._inputRefArray;
    }
    set inputRefArray(inputRefArray: any[]) {
        this._inputRefArray = inputRefArray;
    }
    get isPasting() {
        return this._isPasting;
    }
    set isPasting(isPasting: boolean) {
        this._isPasting = isPasting;
    }
    ...
    public register = async () => {
        ...
        try {
            if (
                await ValidatableViewModel.IsSomeInvalid([
                    this.email,
                    this.password,
                ])
            ) {
            ...
            await AppController.Instance.Auth.createAccountForEmailAndPassword(
                e,
                this.password.value,
            );
    ...
    validateCode = async (): Promise<{
        result: boolean;
        hasAccount: boolean;
    }> => {
        const hasAcc = await AppController.Instance.Auth.hasAccount(
            this.email.value,
        );

        const validCode = await AppController.Instance.Auth.validateCode(
            this.email.value,
            this.verificationCodeValue.join(''),
        );
    ...
    public createUser = async (): Promise<{ result: boolean }> => {
        this._inProgress = true;
        const data = await AppController.Instance.Auth.createAccountForEmailAfterVerificationCode(
            this.email.value,
        );
        if (data || data.result) {
