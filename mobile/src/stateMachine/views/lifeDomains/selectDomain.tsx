            ...
            <MasloPage style={this.baseStyles.page} onClose={() => this.cancel()} onBack={() => this.cancel()} theme={this.theme}>
                <Container style={[{ height: this._contentHeight, alignItems: 'center' }]}>
                    ...
                    <View style ={styles.buttonContainer}>
                        {(selectedDomains.length == 2 || selectedDomains.length == 3) && <Button title='Continue' style={styles.continueButton} onPress={() => this.onThreeSelected()} theme={this.theme} />}
                        { selectedDomains.length < 2 && <Button title='No' titleStyles={{color:'black'}} withBorder={true} style={styles.readyButtonNo} onPress={() => this.onEndSurvey()} theme={this.theme} />}
                        { selectedDomains.length < 2 && <Button title='Yes' style={styles.readyButton} onPress={() => this.cancel()}  theme={this.theme} />}
                    </View>
                </Container>
            </MasloPage>
        );
