...
import { StyleSheet, Text, KeyboardAvoidingView, Platform, View, TextInputProps } from 'react-native';
...
import ConfirmAccountViewModel, { FieldTypes } from 'src/viewModels/ConfirmAccountViewModel';
...
const EnabledSteps: StepInfo[] = [
    ...
    Features.Mobile.ConfirmAccount.CollectZipCode ? {
        ...
        request: 'Your zip code will help me find the best jobs in your area',
...
export class ConfirmAccountView extends ViewState<State, ConfirmAccountParams> {
    ...
    constructor(props) {
        ...
        this._steps = EnabledSteps.filter(s => s && (!forceSteps || forceSteps.includes(s.type)));
        if (!this._steps.length) {
            throw new Error('No steps available for ConfirmAccount view');
        }
    }

    private get viewModel() { return Model.value; }
    private get currentStep() { return this._steps[this.state.stepIndex] || this._steps[0]; }
    private get goBackOnDone() { return this.params?.goBackOnDone || false; }

    start() { /* no-op */ }

    private _goToStep = (index: number) => {
        this.contentAnimation = { animation: 'fadeOut', delay: 0, duration: 300 };
        setTimeout(() => {
            this.setState({ stepIndex: index }, () => {
                this.contentAnimation = { animation: 'fadeIn', delay: 0, duration: 300 };
            });
        }, 300);
    }

    submit = async () => this.runLongOperation(async () => {
        ...
        if (nextIndex >= this._steps.length) {
            const result = await this.viewModel.submit(this.params?.forceSteps || undefined);
