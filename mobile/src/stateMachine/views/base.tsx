...
export abstract class ViewState<CState = {}, CParams = any>
    extends React.Component<ViewStateProps<CParams>, CState>
    ...
    private readonly _personaStates: Partial<
        Record<PersonaStateOverriders, Partial<IPersonaViewContext>>
    > = {};
    ...
    protected get persona() {
        return this.props.context.persona;
    }
    ...
    protected get globalLoading() {
        return AppController.Instance.loading;
    }
                    ...
                    (p) => {
                        ...
                        if (p) {
                            ...
                            this.savePersonaState(
                                'globalProgress',
                                PersonaStates.Listen,
                                PersonaViewPresets.Default,
                            );
                        } else {
    ...
    protected trigger<T>(trigger: ScenarioTriggers, params?: T) {
        if (this._breakingPromise) {
            this.logger.log(
                ' ======= SKIPPING TRIGGER, resolving breaking promise',
            );
            this._breakingPromise.resolve();
        } else {
    ...
    protected savePersonaState(
        id: PersonaStateOverriders,
        state: PersonaStates,
        view: PersonaViewState,
    ) {
        this._personaStates[id] = {
            state: this.persona.state,
            view: this.persona.view,
        };
    ...
    protected restorePersonaState(id: PersonaStateOverriders) {
        ...
        const lastState =
            this._lastPersonaStates[this._lastPersonaStates.length - 1] || null;

        if (lastState && id !== lastState) {
            this.logger.log(
                'restorePersonaState',
                id,
                ': lastState mismatch, will be restored by',
                lastState,
            );
    ...
    showModal(settings: ModalProps, skipHide: boolean = false) {
        ...
        transaction(() => {
            ...
            const cfg = this.persona.getContainerHeight(
                ModalView.MinContentHeight,
            );
    ...
    protected get contentContainerStyles() {
    ...
    protected runLongOperation = async <T extends any>(
        ...
        if (this._personaStates.longOperation) {
            this.logger.log(
                "WARNING: Runnig another long operation while previous hasn't been finished yet!",
            );
        }
    ...
    render() {
        const { animation = null, duration = 0, delay = 0 } =
            this.contentAnimation || {};

        const hasProgress =
            this._progressCounter > 0 ||
            (this.enableGlobalProgressTracking && this.globalLoading);
        const contentContainerAnimation: Animatable.Animation = hasProgress
            ? 'fadeOut'
            : 'fadeIn';
        ...
        return (
            <>
                {!this.modalSettings && (
                    <Animatable.View
                        animation={contentContainerAnimation}
                        duration={400}
                        style={this.contentContainerStyles}>
                        <Animatable.View
                            animation={animation}
                            duration={duration}
                            delay={delay}>
