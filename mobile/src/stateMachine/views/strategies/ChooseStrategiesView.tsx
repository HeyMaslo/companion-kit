    ...
    return (
      <MasloPage style={this.baseStyles.page} onClose={() => this.onClose()} onBack={() => this.onBack()} theme={this.theme}>
        <Container style={[{ height: this._contentHeight, paddingTop: 10 }]}>
          ...
          <Button title='SELECT THESE STRATEGIES' style={styles.selectButton} onPress={this.onSubmit} disabled={this.viewModel.selectedStrategies.length < 1} theme={this.theme} />
        </Container>
      </MasloPage>
    );
  }
