...
export class QolQuestion extends ViewState {
    ...
    constructor(props) {
        ...
        this._contentHeight = this.persona.setupContainerHeight(
            minContentHeight,
            {
                rotation: -140 - this.viewModel.domainNum * 30,
    ...
    private isNextDomain = (currQuestion: number) => {
        return (
            (currQuestion + 1) % this.viewModel.domainQuestionCount === 1 &&
            currQuestion !== 1
        );
    };
    ...
    private animateDomainChange = () => {
        Animated.timing(this.labelState.opacity, {
            ...
            this.persona.view = {
                ...this.persona.view,
                rotation: this.persona.view.rotation - 30,
                transition: { duration: 1 },
            };
            Animated.timing(this.labelState.opacity, {
                toValue: 1,
    ...
    private nextQuestion = (prevResponse: number) => {
        ...
        this.persona.qolArmMagnitudes = {
            ...this.persona.qolArmMagnitudes,
            [this.viewModel.domain]: newDomainMag,
        };
    ...
    private calculateNewDomainMag = (response: number) => {
        ...
        const oldMag: number = this.persona.qolArmMagnitudes[
    ...
    private onClose = (): void | Promise<void> =>
        this.runLongOperation(async () => {
            this.showModal({
                title:
                    'Do you really want to stop the survey? Your progress will be saved.',
                primaryButton: {
                    text: 'yes, stop',
                    action: this.cancel,
                },
    ...
    private checkForInterlude() {
        if (
            this.viewModel.showInterlude &&
            this.viewModel.questionNum === this.viewModel.numQuestions / 2
        ) {
            this.showInterlude();
        }
    }

    private showInterlude = (): void | Promise<void> =>
        this.runLongOperation(async () => {
            this.showModal({
                title: "You're getting there!",
                message:
                    "You're doing great! I have a couple more questions for you.",
                primaryButton: {
                    text: 'CONTINUE',
                    action: this.hideModal,
                },
                ...
                <Container
                    ...
                    <Animated.View style={{ opacity: this.labelState.opacity }}>
                        <Text style={styles.domainLabel}>
                            {this.viewModel.domain.toUpperCase()}
                        </Text>
                    </Animated.View>
                    <View style={styles.subText1}>
                        <Text style={this.textStyles.p3}>
                            {this.viewModel.questionNum + 1} of{' '}
                            {this.viewModel.numQuestions}
                        </Text>
                    </View>
                    <Text style={{ ...this.textStyles.p3, marginTop: '8%' }}>
                        OVER THE LAST 7 DAYS I HAVE...
                    </Text>
                    <View style={styles.question}>
                        <Text style={[this.textStyles.h2, styles.questionText]}>
                    ...
                    <View style={styles.buttonContainer}>
                        <Button
                            ...
                            titleStyles={{ color: Colors.survey.btnFontColor }}
                            withBorder={true}
