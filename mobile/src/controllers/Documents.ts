...
import { DocumentEntry, DocumentLinkShareStatuses, DocumentLinkEntry, Identify, DocumentLinkShareState } from 'common/models';
...
export class DocumentsController extends DocumentsControllerBase implements IDocumentsController {
    ...
    get popupDocument() {
        return this._seendDocsIds
            ...
            ? this.activeLinks.find(d => d.share.status === DocumentLinkShareStatuses.Sent
                && (!this._seendDocsIds[d.id] || this._seendDocsIds[d.id] < d.share.date))
            : null;
    }
    ...
    protected async onItemsProcessed() {
        ...
        this._entries.forEach(d => {
            ...
            if (seenDate && seenDate !== DocumentLinkShareState.getLastStatusDate(d.share, DocumentLinkShareStatuses.Sent)) {
        ...
        this._activeLinks = this._entries
            .filter(e => DocumentEntry.isLink(e)
    ...
    private async saveSeenIds() {
        ...
        await StorageAsync.setValue(ShownDocuemntIdsKey, JSON.stringify(this._seendDocsIds));
    }
}
